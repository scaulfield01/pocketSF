...
if (Meteor.isServer) {
  Meteor.startup(function () {
        ...
        for (var i = 0;  i < response.length;  i++) {
          if (response[i].locations && response[i].title && response[i].release_year && response[i].production_company && response[i].director) {
          ...
          var productionCompany = response[i].production_company;
            ...
            var geo = new GeoCoder({httpAdapter: "https", apiKey: Meteor.settings.turkishCoffee});
